...
public:
    ...
    static std::string generateAnalysisFunction() {
        ...
        ss << "// 分析访存结果\n"
           ...
           << "    \n"
           << "    // 计算变量大小（以元素个数为单位）\n"
           << "    prof->var_size = (prof->end_addr - prof->base_addr + prof->type_size) / prof->type_size;\n"
           << "    \n"
           ...
           << "        if(max_idx != i) {\n"
           << "            // 交换pattern_counts\n"
           << "            unsigned long temp_count = prof->pattern_counts[i];\n"
           << "            prof->pattern_counts[i] = prof->pattern_counts[max_idx];\n"
           << "            prof->pattern_counts[max_idx] = temp_count;\n"
           << "            \n"
           << "            // 同步交换patterns\n"
           << "            unsigned long temp_pattern = prof->patterns[i];\n"
